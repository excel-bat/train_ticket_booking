...
public class BootStrap {
   ...
   */
  static {
    //use try-with-resources don't finally
    try (Reader reader
                 = new InputStreamReader(BootStrap.class.getResourceAsStream("/init.json"), "UTF-8")) {
    ...
    while (sc.hasNextLine()) {
      ...
      try {
        if (ArrayUtils.isNotEmpty(customerInput) && customerInput.length < 4) {
          LOG.info("输入的客户信息如下- CUSTOMER_TYPE:{},DEPARTING_DATE:{},RETURNING_DATE:{}",
                  customerInput[0], customerInput[1], customerInput[2]);
